...
function tab.OnUpdate(vals: {Instance}, labels: {string}, target: TextLabel|TextButton)
	if not target or not (target:IsA('TextLabel') or target:IsA('TextButton')) then warn('OnUpdate requires a Textlabel or TextButton as a Target') end
	if #vals ~= #labels then warn('Failed to mismatch between vals and labels') end
	local function handleType(label: string, val: any): string
		if label:match('Plus$') then
			local base = label:gsub('Plus$', '')
			return base .. ' +' .. tab.format(val)
		else
			return label .. ': ' .. tab.format(val)
		end
	end
	local function updateText()
		...
		for i, v in ipairs(vals) do
			if v:IsA("ValueBase") then
				table.insert(parts, handleType(labels[i], v.Value))
			end
		end
		target.Text = table.concat(parts, "  ")
	end
	for _, v in ipairs(vals) do
		if v:IsA('ValueBase') then
			v.Changed:Connect(updateText)
		end
	end
	updateText()
end
